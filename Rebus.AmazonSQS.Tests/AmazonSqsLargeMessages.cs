...
namespace Rebus.AmazonSQS.Tests
{
    ...
    public class AmazonSqsLargeMessages : SqsFixtureBase
    {
        ...
        public async Task WhenCoreHeadersShouldBeCollapsed_ThenHeadersAreConcatenatedIntoOneAttribute()
        {
            ...
            var inputQueue = _transportFactory.CreateExtended(inputqueueName, "ng-development");

            var outputqueueName = TestConfig.GetName($"outputQueue-{DateTime.Now.Ticks}");
            var outputQueue = _transportFactory.CreateExtended(outputqueueName, "ng-development");

            var largeMessage = File.ReadAllText("../../../largemessages.txt");

            await WithContext(async context =>
            {
                // send the message of to the bus, which will make sure that all core headers
                // are collapsed into a single header
                await outputQueue.Send(inputqueueName, MessageWith(largeMessage), context);
            });
            ...
            await WithContext(async context =>
            {
                ...
                Assert.That(GetStringBody(transportMessage), Is.EqualTo(largeMessage), "Expected to receive the message that we just sent");
